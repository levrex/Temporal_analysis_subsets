...
lf_col # = list with all clinical signatures # (i.e. lymphoid/ myeloid levels) - which indicates baseline profile of patient.
...
for time in df_raw["visnr"].unique():
    ...
    for pid in ids_visit:
        ...
        # -------------------
        # Calculate Ratio Within distances versus Between distances
        # -------------------
        ratio = (
...
plt.xticks([1, 2, 3, 4], ["0", "3", "6", "12"], fontsize=14) # translate visit number to number of months (3, 6, 12)
plt.yticks([0, 1, 2, 3, 4], fontsize=14)
plt.title("Temporal stability of baseline subgroups (k=50)", fontsize=14)
plt.xlabel("Time (months)", fontsize=14) 
plt.ylabel('Silhouette score', fontsize=14)  
plt.legend(fontsize=14)
plt.axhline(0, color='gray', linestyle='--', linewidth=4)
plt.ylim([-.5,1])
plt.grid(True)
